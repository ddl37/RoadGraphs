...
```
The .pycgr_names file will contain road names
"""
...
class RoadGraph:
    ...
    def from_file(path):
        ...
        is_nice_edge = lambda e: len(incident_edges_to[e[0]]) == len(incident_edges_to[e[1]]) == 2
    ...
    def adjacency_list(self):
        ...
        # possible one-directional roads endpoints?
        # assert len(INCIDENCE) == len(self.nodes)

        print(len(self.nodes))
        for i in range(OFFSET, len(self.nodes) + OFFSET):
            # if not len(INCIDENCE[i]):
